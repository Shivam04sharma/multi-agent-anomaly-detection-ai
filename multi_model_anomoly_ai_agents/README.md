...
**File:** `src/services/validation_agent.py`
...
**File:** `src/services/feature_builder.py`
...
**File:** `src/services/detection_engine.py`
...
**File:** `src/services/explanation_engine.py`
...
DB_SCHEMA=anomaly
VAULT_PROXY_URL=http://localhost:8200
...
Open `index.html` directly in your browser — no server needed for the UI.

Or open Swagger docs at: `http://localhost:9005/docs`
...
| `VAULT_PROXY_URL` | No | `http://localhost:8200` | Vault proxy URL for DB credentials |
...
- Sidebar navigation for all 5 endpoints
- Editable base URL (top bar) — change if server runs on different port
- Analyze / Upload → Stats cards + Anomaly cards with severity badges
- Predict → Anomaly YES/NO + Score + Explanation
